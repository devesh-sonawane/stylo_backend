...
# System template for the fashion assistant
SYSTEM_TEMPLATE = """
...
def chat():
    """
    API endpoint for chatting with the fashion assistant
    ...
    try:
        ...
        if any(word in query.lower() for word in ['thank', 'thanks', 'appreciate', 'helpful']):
            ...
            search_query = query
            if len(chat_history) > 2 and not query.lower().startswith(('i need', 'suggest', 'recommend', 'show me', 'find me')):
            ...
            if len(results) == 0 or results[0][1] < 0.6:
                ...
                for doc, _ in results:
                    ...
                    product = {
                        ...
                        "product_link": metadata.get("Product Link", ""),
                        "category": metadata.get("Category", ""),
                        "colors": metadata.get("Colors", "")
                    }
                    # Only add products with valid links and names
                    if product["product_link"] and product["name"] and product["product_link"] != "No link available":
                        # Check if this product is already in the list (avoid duplicates)
                ...
                prompt = ChatPromptTemplate.from_messages([
                    ...
                    *chat_history[1:],  # Skip the system message as we're adding it explicitly above
                    HumanMessage(content=f"Here is product information that might be relevant to the user's request. Remember that the original request was about: '{original_query}'\n\n{context_text}")
                ])
